...
const buildRaceContext = (race) => {
  ...
  const traits = {
    speed: bias?.traits.speed ?? profile.speed,
    power: bias?.traits.power ?? Math.min(1, profile.power + (dirt ? 0.25 : 0)),
    stamina: bias?.traits.stamina ?? profile.stamina,
    sustain: bias?.traits.sustain ?? profile.sustain,
  };

  const course = race?.course ?? race?.track ?? null;
  const summary = bias?.summary ?? `${course ?? "開催場未取得"}${surface ?? ""}${distance || "-"}m。${profile.label}条件として評価します。`;

  return {
    raceId: race?.id ?? `${race?.raceDate ?? "unknown"}-${course ?? "unknown"}-${race?.raceNo ?? race?.number ?? 0}R`,
