...
const valueTextFor = (horse, value) => {
  ...
  return `単勝${horse.odds.winOdds}倍・${horse.odds.popularity}人気をValue評価に反映`;
};
...
const buildVerdictPayload = ({
  ...
  const abilityText = horse.odds?.zi
    ? `ZI ${horse.odds.zi}を能力評価に反映`
    : `近走${horse.pastRuns?.length ?? 0}走から能力を評価`;
  ...
  return {
    ...
    analysis: {
      ...
      confidenceReasons: [
        ...
        trainingStatus,
        horse.pedigree ? "4代血統を評価に使用" : "血統情報は一部未取得",
        contextSummary,
        valueText,
      ],
      ...
      factorsDetail: {
        ...
        training: { key: "training", label: "調教", score: training, maxScore: 100, status: trainingAnalysis.count ? "active" : "missing", summary: trainingReadable, evidence: trainingAnalysis.strengths ?? [] },
        course: { key: "course", label: "コース", score: course, maxScore: 100, status: "active", summary: contextSummary, evidence: [`コース適性は${factorLabel(course)}`, `距離適性は${factorLabel(factors.distance)}`] },
        pace: { key: "pace", label: "展開", score: pace, maxScore: 100, status: horse.pastRuns?.length ? "active" : "missing", summary: "近走の通過順と位置取り傾向を評価", evidence: [`展開適性は${factorLabel(pace)}`, `上がり評価は${factorLabel(factors.lap)}`] },
        stable: { key: "stable", label: "厩舎", score: stable, maxScore: 100, status: "active", summary: "所属・調教取得状況を補助評価", evidence: [`厩舎補助評価は${factorLabel(stable)}`] },
        form: { key: "form", label: "近走", score: form, maxScore: 100, status: horse.pastRuns?.length ? "active" : "missing", summary: "着順・着差・近走推移を評価", evidence: [`近走評価は${factorLabel(form)}`, abilityText] },
        value: { key: "value", label: "妙味", score: value, maxScore: 100, status: value == null ? "missing" : "active", summary: valueText, evidence: value == null ? ["オッズ未取得"] : [`Value評価は${factorLabel(value)}`] },
      },
      insight,
      pros: [
        `${abilityText}。能力評価は${factorLabel(ability)}`,
        `${bloodSummary}`,
        trainingAnalysis.count ? `${trainingAnalysis.summary}` : "調教時計は未取得のため評価控えめ",
      ],
      cons: [
        value == null ? "オッズ未取得のため妙味は未評価" : `人気とオッズのバランスは${factorLabel(value)}`,
        trainingAnalysis.count ? "調教は取得範囲内での判定" : "調教時計が不足",
      ],
