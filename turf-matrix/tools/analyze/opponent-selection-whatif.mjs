...
const stats = Object.fromEntries(Object.keys(methods).map((key) => [key, { count: 0, opponentPlaced: 0, axisPlaced: 0, coPlaced: 0 }]));
const raceRows = [];
let skipped = 0;
...
const lines = [
  ...
  ...Object.entries(methods).map(([key, method]) => {
    const row = stats[key];
    return `| ${method.label} | ${row.count} | ${pct(row.opponentPlaced, row.count)} | ${pct(row.axisPlaced, row.count)} | ${pct(row.coPlaced, row.count)} |`;
  }),
  "",
  ...
  rankedMethods[0]?.[1].count
    ? `現時点の同時3着内率トップは **${methods[rankedMethods[0][0]].label}**。ただしワイド払戻未取得のため、採用判断は的中率ベースの暫定評価。`
