...
const compactHorse = (horse) => ({
  number: horse.number,
  name: horse.name,
  currentTraining: horse.currentTraining,
  shadowTraining: horse.shadowTraining,
  trainingAdjustment: horse.trainingAdjustment,
  currentTm: horse.currentTm,
  shadowTm: horse.shadowTm,
});

const buildRaceTrainingEvidencePrediction = (race) => {
  ...
  const horses = (race.horses ?? []).map((horse) => {
    ...
    if (currentTraining == null || currentTm == null) return null;
    const profile = buildTrainingProfile(horse);
    const shadow = buildTrainingEvidenceShadow(profile, currentTraining, "empiricalQuality", {
    ...
    return {
      ...
      currentTraining,
      shadowTraining: shadow.shadowScore,
      trainingAdjustment: shadow.adjustment,
      currentTm,
      shadowTm: clamp(currentTm + tmDelta, 45, 92),
      tmDelta,
      hasFinal: shadow.hasFinal ?? false,
      hasOneWeek: shadow.hasOneWeek ?? false,
      sessionCount: shadow.sessionCount ?? 0,
    };
  }).filter(Boolean);
  const currentTrainingLeader = ranked(horses, "currentTraining")[0] ?? null;
  const shadowTrainingLeader = ranked(horses, "shadowTraining")[0] ?? null;
  ...
  return {
    ...
    currentTrainingLeader: currentTrainingLeader ? compactHorse(currentTrainingLeader) : null,
    shadowTrainingLeader: shadowTrainingLeader ? compactHorse(shadowTrainingLeader) : null,
    ...
    trainingLeaderChanged: currentTrainingLeader?.name !== shadowTrainingLeader?.name,
...
const evaluateRaceTrainingEvidencePrediction = (prediction, resultRace) => {
  ...
  const currentTrainingLeader = ranked(rows, "currentTraining")[0];
  const shadowTrainingLeader = ranked(rows, "shadowTraining")[0];
  ...
  return {
    ...
    adjustedHorseCount: rows.filter((horse) => horse.trainingAdjustment !== 0).length,
    trainingLeaderChanged: currentTrainingLeader.name !== shadowTrainingLeader.name,
    tmLeaderChanged: currentTmLeader.name !== shadowTmLeader.name,
    currentTrainingLeader: { ...compactHorse(currentTrainingLeader), finish: currentTrainingLeader.finish },
    shadowTrainingLeader: { ...compactHorse(shadowTrainingLeader), finish: shadowTrainingLeader.finish },
    ...
    currentTrainingPairwise: pairwise(rows, "currentTraining"),
    shadowTrainingPairwise: pairwise(rows, "shadowTraining"),
    currentTmPairwise: pairwise(rows, "currentTm"),
    shadowTmPairwise: pairwise(rows, "shadowTm"),
    maxAbsAdjustment: Math.max(0, ...rows.map((horse) => Math.abs(horse.trainingAdjustment))),
  };
};

const aggregateTrainingEvidenceEvaluation = (races) => {
  ...
  const currentTraining = rate("current", "Training");
  const shadowTraining = rate("shadow", "Training");
  ...
  return {
    ...
    trainingLeaderChangedRaceCount: sum((race) => Number(race.trainingLeaderChanged)),
    tmLeaderChangedRaceCount: sum((race) => Number(race.tmLeaderChanged)),
    currentTrainingWins: sum((race) => Number(race.currentTrainingLeader.finish === 1)),
    shadowTrainingWins: sum((race) => Number(race.shadowTrainingLeader.finish === 1)),
    currentTrainingPlaces: sum((race) => Number(race.currentTrainingLeader.finish <= 3)),
    shadowTrainingPlaces: sum((race) => Number(race.shadowTrainingLeader.finish <= 3)),
    ...
    currentTrainingPairwiseRate: currentTraining.rate,
    shadowTrainingPairwiseRate: shadowTraining.rate,
...
export {
  ...
  buildRaceTrainingEvidencePrediction,
