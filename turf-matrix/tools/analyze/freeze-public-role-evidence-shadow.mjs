...
const MODEL_VERSION = "public-role-evidence-shadow-v4";
...
const compactHorse = (horse, race, evidence = null) => {
  ...
  return {
    ...
    indexRank: rank,
    ...(evidence ? {
      marketGap: evidence.marketGap,
      qualityScore: evidence.qualityScore,
      foundationScore: evidence.foundationScore,
      conditionScore: evidence.conditionScore,
      evidenceRoleQuality: evidence.evidenceRoleQuality,
      dangerStrength: evidence.dangerStrength,
      supports: evidence.supports,
      risks: evidence.risks,
      protections: evidence.protections,
      riskPoints: evidence.riskPoints,
      protectionPoints: evidence.protectionPoints,
...
const predictions = (source.races ?? []).map((race) => {
  ...
  return {
    ...
    paceValue: compactHorse(selected.paceValue, race),
    evidenceValue: compactHorse(selected.evidenceValue, race, selected.evidence.value),
    productionDanger: compactHorse(selected.productionDanger, race),
    paceDanger: compactHorse(selected.paceDanger, race),
    evidenceDanger: compactHorse(selected.evidenceDanger, race, selected.evidence.danger),
  };
});
...
const artifact = {
  ...
  summary: {
    ...
    evidenceValueCount: predictions.filter((race) => race.evidenceValue).length,
    evidenceDangerCount: predictions.filter((race) => race.evidenceDanger).length,
    valueAbstainedCount: predictions.filter((race) => race.paceValue && !race.evidenceValue).length,
    dangerAbstainedCount: predictions.filter((race) => race.paceDanger && !race.evidenceDanger).length,
    valueChangedCount: predictions.filter((race) => race.paceValue?.number !== race.evidenceValue?.number).length,
    dangerChangedCount: predictions.filter((race) => race.paceDanger?.number !== race.evidenceDanger?.number).length,
  },
  predictionSha256,
  predictions,
};
...
if (!dryRun && existsSync(output)) {
  ...
  if (previous.predictionSha256 !== predictionSha256) throw new Error(`Frozen public-role Evidence shadow differs: ${output}`);
