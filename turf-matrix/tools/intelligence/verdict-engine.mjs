    ...
    analysis: {
      ...
      factorsDetail: {
        ...
        training: {
          ...
          score: training,
          maxScore: 100,
          status: trainingAnalysis.status ?? (trainingAnalysis.count ? "active" : "missing"),
          confidence: trainingAnalysis.confidence ?? "low",
        ...
        stable: {
          ...
          score: stable,
          maxScore: 100,
          status: stableAnalysis?.status ?? "missing",
          confidence: stableAnalysis?.confidence ?? "low",
          summary: stableAnalysis?.summary ?? "調教師・ローテーション・騎手起用を陣営運用として評価",
          evidence: stableAnalysis?.evidence ?? [`厩舎補助評価は${factorLabel(stable)}`],
          components: stableAnalysis?.components ?? {},
          rotation: stableAnalysis?.rotation ?? null,
          jockey: stableAnalysis?.jockey ?? null,
          travel: stableAnalysis?.travel ?? null,
          stablePattern: stableAnalysis?.stablePattern ?? null,
        },
        form: {
          key: "form",
          label: "近走",
